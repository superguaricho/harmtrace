...
data TMinBorrow
    = Tm_21_bor (SD MinMode I MinClass)
...
data Base_SD deg clss n where
    Base_SD ::
        TritMinVSub deg clss -> -- Min5    deg clss  n
        Base_SD deg clss (Su n)
    -- Rule for explaining perfect secondary dominants
    Cons_Vdom ::
        Base_SD (VDom deg) DomClass n ->
        Base_SD deg clss n ->
        Base_SD deg clss (Su n)
    Cons_Diat ::
        Base_SD (DiatV deg) MinClass n ->
        Base_SD deg MinClass n ->
        Base_SD deg MinClass (Su n)
    Cons_DiatM ::
        Base_SD (DiatVM deg) MajClass n ->
        Base_SD deg MajClass n ->
    ...
    Cons_Vmin ::
        Base_SD (VMin deg) MinClass n ->
        Base_SD deg DomClass n ->
...
data Base_Final deg clss n where
    ...
    Final_Tritone ::
        Base_Final (Tritone deg) DomClass n ->
        Base_Final deg DomClass (Su n)
    Final_Dim_V ::
        Base_Final (IIbDim deg) DimClass n ->
        Base_Final deg DomClass (Su n)
...
data Surface_Chord deg clss n where
    ...
    Dim_Chord_Trns ::
        Surface_Chord (MinThird deg) DimClass n ->
