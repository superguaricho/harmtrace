...
import HarmTrace.HAnTree.HAn hiding (PD, PT)
...
$( join
    <$> mapM
        (\t -> gadtInstance ''ParseG t 'parseG 'parseGdefault)
        allTypes
 )

$( join
    <$> mapM
        (\t -> simplInstance ''GTree t 'gTree 'gTreeDefault)
        allTypes
 )
...
instance {-# OVERLAPPING #-} ParseG (Base_SD deg clss Ze) where parseG = empty
...
instance {-# OVERLAPPING #-}
    ...
    ParseG (Base_SD deg MinClass (Su n))
    where
    parseG =
        Base_SD
            <$> parseG
...
instance {-# OVERLAPPING #-}
    ...
    ParseG (Base_SD deg MajClass (Su n))
    where
    parseG =
        Base_SD
            <$> parseG
...
instance {-# OVERLAPPING #-}
    ...
    ParseG (Base_SD deg DomClass (Su n))
    where
    parseG =
        Base_SD
            <$> parseG
...
instance {-# OVERLAPPING #-}
    ...
    ParseG (Base_SD deg DimClass (Su n))
    where
    parseG =
        Base_SD
            <$> parseG
...
instance {-# OVERLAPPING #-} ParseG (Base_Final deg clss Ze) where parseG = empty

instance {-# OVERLAPPING #-}
    (ParseG (FinalDimTrans deg clss)) =>
    ParseG (Base_Final deg clss (Su n))
    where
    parseG = Base_Final <$> parseG

instance {-# OVERLAPPING #-}
    ( ParseG (FinalDimTrans deg DomClass)
    , ParseG (FinalDimTrans deg MinClass)
    , ParseG (Base_Final (Tritone deg) DomClass n)
    , ParseG (Base_Final (IIbDim deg) DimClass n)
    ) =>
    ParseG (Base_Final deg DomClass (Su n))
    where
    parseG =
        Base_Final
            <$> parseG
...
instance {-# OVERLAPPING #-} ParseG (Surface_Chord deg clss Ze) where parseG = empty

instance {-# OVERLAPPING #-}
    ( ToDegree deg
    , ParseG (Surface_Chord (MinThird deg) DimClass n)
    ) =>
    ParseG (Surface_Chord deg DimClass (Su n))
    where
    parseG =
        Dim_Chord_Trns
            <$> parseG
...
instance {-# OVERLAPPING #-}
    ...
    ParseG (Surface_Chord deg clss (Su n))
    where
...
-- General case
pChord deg clss = setStatus <$> pSatisfy recognize insertion
  where
    {-# INLINE recognize #-}
...
toGTree con transp deg =
    [ Node
        (HAnTrans . con 1 $ toTransSDVal transp deg)
        (gTree deg)
        Nothing
    ]
...
toGTreeSplit ::
    (GetDegree a, GetDegree b, GTree a, GTree b) =>
...
instance {-# OVERLAPPING #-}
    ( GTree (Base_SD (VDom deg) DomClass n)
...
instance {-# OVERLAPPING #-}
    ...
    GTree (Base_Final deg clss (Su n))
    where
...
instance {-# OVERLAPPING #-}
    ...
    GTree (Surface_Chord deg clss (Su n))
    where
...
instance {-# OVERLAPPABLE #-} GetDegree (Base_Final deg clss n) where
...
instance {-# OVERLAPPING #-}
    (GetDegree (Surface_Chord (MinThird deg) DimClass n)) =>
    GetDegree (Surface_Chord deg clss (Su n))
    where
